...
| phylum | Spirochaetota | 203691 |
| class | Spirochaetia | 203692 |
| order | Spirochaetales | 136 |
| family | Treponemataceae | 2845253 |
| *genus* | Treponema | 157 |

### Species with assemblies 具有组装的物种

Treponemataceae是细菌界中的一个科

```shell
cd /mnt/c/shengxin
mkdir -p data/Treponema/summary
cd /mnt/c/shengxin/data/Treponema/summary

#找出和密螺旋体同一科(Treponemaceae)的所有属

# should have a valid name of genus
nwr member Treponemataceae -r genus |
    grep -v -i "Candidatus " |
    grep -v -i "candidate " |
    grep -v " sp." |
    grep -v " spp." |
...
cat raw.tsv |
    ...
    > Treponema.assembly.tsv
 #创建简写名称的密螺旋体属水平的各菌株基因组下载文件

 datamash check < Treponema.assembly.tsv
 #1166 lines, 5 fields

# find potential duplicate strains or assemblies 
cat Treponema.assembly.tsv |
...
mkdir -p ../assembly/
cp Treponema.assembly.tsv /mnt/c/shengxin/data/Treponema/assembly/
...
```shell
cd /mnt/c/shengxin/data/Treponema

nwr template summary/Treponema.assembly.tsv \
    --count \
    --rank genus
...
#strains.taxon.tsv内容
B_paras_RmG11_GCF_022376835_1   Brucepastera parasyntrophica    Brucepastera    Treponemataceae Spirochaetales  Spirochaetia
T_ber_ATCC_BAA_909_GCF_900167025_1      Treponema berlinense    Treponema       Treponemataceae Spirochaetales  Spirochaetia

# genus.lst and genus.count.tsv
bash Count/rank.sh

#genus.lst 文件只有一列，提取了属的名称
#genus.count.tsv 统计密螺旋体属以及各属的species和strains去重后的数量
...
```shell
cd /mnt/c/shengxin/data/Treponema
nwr template ../Treponema/assembly/Treponema.assembly.tsv\
    --ass
...
#1131257 1139690.5       2827    1139589.5       1       33.9
...
```bash
rsync -avP \
    /mnt/c/shengxin/data/Treponema/assembly/ \
    wangq@202.119.37.251:qyl/data/Treponema/ASSEMBLY
#本地运行

rsync -avP \
    /mnt/c/shengxin/data/Treponema/summary/ \
    wangq@202.119.37.251:qyl/data/Treponema/summary
```
...
cp ASSEMBLY/Treponema.assembly.tsv summary/
nwr template /mnt/c/shengxin/data/Treponema/assembly/Treponema.assembly.tsv \
    --bs

head BioSample/sample.tsv
#SAMD00000356    Parab_fer_NBRC_106233_GCF_000685035_1   ParaTreponema_ferrariae
#SAMD00000357    Parab_mim_NBRC_106338_GCF_000739815_1   ParaTreponema_mimosarum
...
Parab_fer_NBRC_106233_GCF_000685035_1   SAMD00000356    NBRC 106233     free living             Iron Mine       Brazil:Minas Gerais     17012573                BFE01S          Treponema ferrariae NBRC 106233 genome sequencing project   NBRC 106233      heterotroph     NBRC 106233

rsync -avP \
    /mnt/c/shengxin/data/Treponema/Biosample/ \
    wangq@202.119.37.251:qyl/data/Treponema/Biosample
```
...
nwr template /mnt/c/shengxin/data/Treponema/summary/Treponema.assembly.tsv \
    --mh \
    --parallel 16 \
...
rsync -avP \
    wangq@202.119.37.251:/share/home/wangq/qyl/data/Treponema/Biosample/ \
    /mnt/c/shengxin/data/Treponema/Biosample

rsync -avP \
    wangq@202.119.37.251:/share/home/wangq/qyl/data/Treponema/MinHash/ \
    /mnt/c/shengxin/data/Treponema/MinHash
    #本地运行   
...
```shell
mkdir -p cd /mnt/c/shengxin/data/Treponema/tree
cd /mnt/c/shengxin/data/Treponema/tree
...
nw_display -s -b 'visibility:hidden' -w 1200 -v 20 minhash.species.newick |
    rsvg-convert -o Treponema.minhash.png
#报错：超过了 32767 像素，这是 rsvg-convert 当前无法处理的最大尺寸限制。
nw_display -s -b 'visibility:hidden' -w 800 -v 15 minhash.species.newick | rsvg-convert -o Treponema.minhash.png
#还是过大
...
| Treponemataceae | Brucepastera | Brucepastera parasyntrophica | 1 |
|  | Treponema | Treponema berlinense | 1 |
|  |  | Treponema denticola | 28 |
|  |  | Treponema lecithinolyticum | 1 |
|  |  | Treponema maltophilum | 1 |
|  |  | Treponema medium | 3 |
|  |  | Treponema pallidum | 494 |
|  |  | Treponema paraluiscuniculi | 1 |
|  |  | Treponema parvum | 3 |
|  |  | Treponema pectinovorum | 3 |
|  |  | Treponema pedis | 3 |
|  |  | Treponema peruense | 1 |
|  |  | Treponema phagedenis | 14 |
|  |  | Treponema porcinum | 3 |
|  |  | Treponema primitia | 1 |
|  |  | Treponema putidum | 5 |
|  |  | Treponema rectale | 2 |
|  |  | Treponema ruminis | 2 |
|  |  | Treponema saccharophilum | 1 |
|  |  | Treponema socranskii | 2 |
|  |  | Treponema sp. | 6 |
|  |  | Treponema sp. B152 | 1 |
|  |  | Treponema sp. C6A8 | 1 |
|  |  | Treponema sp. CETP13 | 1 |
|  |  | Treponema sp. Marseille-Q3903 | 1 |
|  |  | Treponema sp. Marseille-Q4130 | 1 |
|  |  | Treponema sp. Marseille-Q4132 | 1 |
|  |  | Treponema sp. OMZ 305 | 1 |
|  |  | Treponema sp. OMZ 787 | 1 |
|  |  | Treponema sp. OMZ 788 | 1 |
|  |  | Treponema sp. OMZ 789 | 1 |
|  |  | Treponema sp. OMZ 790 | 1 |
|  |  | Treponema sp. OMZ 791 | 1 |
|  |  | Treponema sp. OMZ 792 | 1 |
|  |  | Treponema sp. OMZ 798 | 1 |
|  |  | Treponema sp. OMZ 799 | 1 |
|  |  | Treponema sp. OMZ 803 | 1 |
|  |  | Treponema sp. OMZ 838 | 1 |
|  |  | Treponema sp. OMZ 855 | 1 |
|  |  | Treponema sp. OMZ 857 | 1 |
|  |  | Treponema sp. OMZ 906 | 1 |
|  |  | Treponema sp. UBA1226 | 1 |
|  |  | Treponema sp. UBA2879 | 1 |
|  |  | Treponema sp. UBA4267 | 1 |
|  |  | Treponema sp. UBA4873 | 1 |
|  |  | Treponema sp. UBA5919 | 1 |
|  |  | Treponema sp. UBA6326 | 1 |
|  |  | Treponema sp. UBA6337 | 1 |
|  |  | Treponema sp. UBA6355 | 1 |
|  |  | Treponema sp. UBA6367 | 1 |
|  |  | Treponema sp. UBA6479 | 1 |
|  |  | Treponema sp. UBA7579 | 1 |
|  |  | Treponema sp. UBA7863 | 1 |
|  |  | Treponema succinifaciens | 2 |
|  |  | Treponema vincentii | 5 |
|  |  | Treponema zuelzerae | 1 |
...
```shell
#TIGRFAM
mkdir -p mnt/shengxin/data/Treponema/HMM/TIGRFAM
cd mnt/shengxin/data/Treponema/HMM/TIGRFAM
...
mkdir -p /mnt/c/shengxin/data/Treponema/HMM/bac120
cd /mnt/c/shengxin/data/Treponema/HMM/bac120


#下载bac120.tsv文件
mnt/shengxin/data/Treponema/HMM/bac120/

mkdir -p HMM

cat /mnt/c/shengxin/data/Treponema/HMM/bac120/bac120.tsv |
    sed '1d' |
    tsv-select -f 1 |
    grep '^TIGR' |
...
nw_display -s -b 'visibility:hidden' -w 1200 -v 20 Protein/bac120.reroot.newick |
    rsvg-convert -o tree/Treponema.marker.png

rsync -avP \
    /mnt/c/shengxin/data/Treponema/HMM/ \
    wangq@202.119.37.251:/share/home/wangq/qyl/data/Treponema/HMM
```