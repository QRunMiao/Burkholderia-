...
| family | Clostridiaceae | 31979 |
| *genus* | Clostridium | 1485 |

### Species with assemblies 具有组装的物种

Clostridiaceae 是细菌界中的一个科

```shell
cd /mnt/c/shengxin
mkdir -p data/Clostridium/summary
cd /mnt/c/shengxin/data/Clostridium/summary

#找出和梭菌同一科(Clostridiaceae)的所有属

# should have a valid name of genus
nwr member Clostridiaceae -r genus |
    grep -v -i "Candidatus " |
    grep -v -i "candidate " |
    grep -v " sp." |
    grep -v " spp." |
...
cat raw.tsv |
    ...
    > Clostridium.assembly.tsv
 #创建简写名称的梭菌属水平的各菌株基因组下载文件

 datamash check < Clostridium.assembly.tsv
 #5204 lines, 5 fields

# find potential duplicate strains or assemblies 
cat Clostridium.assembly.tsv |
...
cd ..
mkdir -p  assembly/
cp summary/Clostridium.assembly.tsv /mnt/c/shengxin/data/Clostridium/assembly/
...
```shell
cd /mnt/c/shengxin/data/Clostridium

nwr template summary/Clostridium.assembly.tsv \
    --count \
    --rank genus
...
head Count/strains.taxon.tsv
#strains.taxon.tsv内容
Ab_fermenta_MCWD3_GCF_001559865_1       Abyssisolibacter fermentans     Abyssisolibacter        Clostridiaceae  Eubacteriales   Clostridia
Al_fla_MSJ_5_GCF_018919215_1    Alkaliphilus flagellatus        Alkaliphilus    Clostridiaceae  Eubacteriales   Clostridia

# genus.lst and genus.count.tsv
bash Count/rank.sh

#genus.lst 文件只有一列，提取了属的名称
#genus.count.tsv 统计梭菌属以及各属的species和strains去重后的数量
...
| Anaeromonas | 2 | 2 |
| Anaerophilus | 1 | 1 |
| Anaerosolibacter | 2 | 2 |
| Anoxynatronum | 1 | 1 |
| Beduini | 1 | 2 |
| Butyricicoccus | 37 | 82 |
| Caloramator | 6 | 6 |
| Clostridium | 1 | 1 |
...
```shell
cd /mnt/c/shengxin/data/Clostridium
nwr template ../Clostridium/assembly/Clostridium.assembly.tsv\
    --ass
...
cp -r data/Bacteria/ASSEMBLY/Pectobacterium_* ~/qyl/data/Clostridium
cp -r data/Bacteria/ASSEMBLY/Hungatella_* ~/qyl/data/Clostridium
cp -r data/Bacteria/ASSEMBLY/Clostridium_* ~/qyl/data/Clostridium

#下载到本地
rsync -avP \
wangq@202.119.37.251:qyl/data/Clostridium/ASSEMBLY \
/mnt/c/shengxin/data/Clostridium/assembly
...
#1129905 1904547 2283    1139123 2       359.2
...
```bash
rsync -avP \
    /mnt/c/shengxin/data/Clostridium/Count/ \
    wangq@202.119.37.251:qyl/data/Clostridium/Count
#本地运行

rsync -avP \
    /mnt/c/shengxin/data/Clostridium/summary/ \
    wangq@202.119.37.251:qyl/data/Clostridium/summary
```
...
cp ASSEMBLY/Clostridium.assembly.tsv summary/
nwr template /mnt/c/shengxin/data/Clostridium/assembly/Clostridium.assembly.tsv \
    --bs

head BioSample/sample.tsv
#SAMD00000356    Parab_fer_NBRC_106233_GCF_000685035_1   ParaClostridium_ferrariae
#SAMD00000357    Parab_mim_NBRC_106338_GCF_000739815_1   ParaClostridium_mimosarum
...
Parab_fer_NBRC_106233_GCF_000685035_1   SAMD00000356    NBRC 106233     free living             Iron Mine       Brazil:Minas Gerais     17012573                BFE01S          Clostridium ferrariae NBRC 106233 genome sequencing project   NBRC 106233      heterotroph     NBRC 106233

rsync -avP \
    /mnt/c/shengxin/data/Clostridium/Biosample/ \
    wangq@202.119.37.251:qyl/data/Clostridium/Biosample
```
...
nwr template /mnt/c/shengxin/data/Clostridium/summary/Clostridium.assembly.tsv \
    --mh \
    --parallel 16 \
...
rsync -avP \
    wangq@202.119.37.251:/share/home/wangq/qyl/data/Clostridium/Biosample/ \
    /mnt/c/shengxin/data/Clostridium/Biosample

rsync -avP \
    wangq@202.119.37.251:/share/home/wangq/qyl/data/Clostridium/MinHash/ \
    /mnt/c/shengxin/data/Clostridium/MinHash
    #本地运行   
...
```shell
mkdir -p cd /mnt/c/shengxin/data/Clostridium/tree
cd /mnt/c/shengxin/data/Clostridium/tree
...
nw_display -s -b 'visibility:hidden' -w 1200 -v 20 minhash.species.newick |
    rsvg-convert -o Clostridium.minhash.png
#报错：超过了 32767 像素，这是 rsvg-convert 当前无法处理的最大尺寸限制。
nw_display -s -b 'visibility:hidden' -w 800 -v 15 minhash.species.newick | rsvg-convert -o Clostridium.minhash.png
#还是过大
...
| --- | --- |
| Proteins | 10,077,502 |
| Unique headers and annotations | 5,237,323 |
| Unique proteins | 5,199,141 |
| all.replace.fa | 10,077,502 |
| all.annotation.tsv | 10,077,503 |
| all.info.tsv | 10,077,503 |
...
```shell
#TIGRFAM
mkdir -p mnt/shengxin/data/Clostridium/HMM/TIGRFAM
cd mnt/shengxin/data/Clostridium/HMM/TIGRFAM
...
mkdir -p /mnt/c/shengxin/data/Clostridium/HMM/bac120
cd /mnt/c/shengxin/data/Clostridium/HMM/bac120


#下载bac120.tsv文件
mnt/shengxin/data/Clostridium/HMM/bac120/

mkdir -p HMM

cat /mnt/c/shengxin/data/Clostridium/HMM/bac120/bac120.tsv |
    sed '1d' |
    tsv-select -f 1 |
    grep '^TIGR' |
...
nw_display -s -b 'visibility:hidden' -w 1200 -v 20 Protein/bac120.reroot.newick |
    rsvg-convert -o tree/Clostridium.marker.png

rsync -avP \
    /mnt/c/shengxin/data/Clostridium/HMM/ \
    wangq@202.119.37.251:/share/home/wangq/qyl/data/Clostridium/HMM
```